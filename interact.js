...
// List contract methods and signatures
// contract.interface.fragments.forEach((fragment) => {
//   if (fragment.type === "function") {
//     console.log(`Function: ${fragment.name}, Signature: ${fragment.format()}`);
//   }
// });
...
const getAllVoters = async () => {
  try {
    const [voterAddresses, hasVoted, isValidated, votedCandidates] =
      await contract.getAllVoters();
    for (let i = 0; i < voterAddresses.length; i++) {
      console.log(
        `Voter Address: ${voterAddresses[i]}, Has Voted: ${hasVoted[i]}, Is Validated: ${isValidated[i]}, Voted Candidate: ${votedCandidates[i]}`
      );
    }
  } catch (error) {
